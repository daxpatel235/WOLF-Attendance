...
import { ArrowRight, ArrowLeft, User, Sparkles, Backpack, GraduationCap, Check } from "lucide-react";
...
export function Onboarding() {
  ...
  const complete = async () => {
    setBusy(true);
    const patch = { onboarded: true, firstName: name.trim() || "Student", institutionType: mode === "school" ? "School" : "College" };
...
function VibeCard({ active, onClick, icon, title, desc }: { active: boolean; onClick: () => void; icon: React.ReactNode; title: string; desc: string }) {
  return (
    <motion.button whileHover={{ y: -4 }} whileTap={{ scale: 0.97 }} onClick={onClick}
      className={`relative text-left p-5 rounded-[var(--r-lg)] border-2 transition-all overflow-hidden ${active ? "border-[var(--accent)] bg-[var(--accent-soft)]" : "border-[var(--border)] bg-[var(--surface-2)] hover:border-[var(--border-strong)]"}`}>
      ...
      <div className={`w-12 h-12 rounded-[var(--r)] grid place-items-center mb-3 ${active ? "bg-[image:var(--grad)] text-white shadow-[var(--shadow-glow)]" : "bg-[var(--surface-3)] text-[var(--text-2)]"}`}>{icon}</div>
